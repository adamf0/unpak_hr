...
class GetAllLaporanAbsenQueryHandler extends Query
{
    ...
    public function handle(GetAllLaporanAbsenQuery $query)
    {
        ...
        $start = empty($query->GetTanggalMulai())? date('Y-m-01'):$query->GetTanggalMulai();
        $end = empty($query->GetTanggalAkhir())? date('Y-m-t'):$query->GetTanggalAkhir();
        ...
        for ($date = Carbon::parse($query->GetTanggalMulai())->setTimezone('Asia/Jakarta')->startOfMonth(); $date->lte($end); $date->addDay()) {
