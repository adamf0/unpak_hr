...
class DatatableSPPDController extends Controller
{
    ...
    public function index(Request $request){
        ...
        $q = new GetAllSPPDQuery($verifikasi? null:$nidn,$verifikasi? null:$nip);
        // $q->SetOffset($request->get('start')??null)->SetLimit($request->get('length')??null);
        
        $listSPPD = $this->queryBus->ask($q);
        // dd($listSPPD);
        $listSPPD = $listSPPD
                        ->filter(function($item) use($level,$type,$nidn,$nip){
                            if($level=="sdm"){
        ...
        ->addColumn('action', function ($row) use($level,$nidn,$nip,$verifikasi){
            ...
            if(in_array($level,['dosen','pegawai']) && !$verifikasi){
                ...
                if($row->status=='menunggu'){
                    $render = '<a href="#" class="btn btn-success btn-approve"><i class="bi bi-check-lg"></i></a>';
                    $render .= '
                        <a href="#" class="ml-2 btn btn-danger btn-reject"><i class="bi bi-x-lg"></i></a>
                    ';
                }
                else if($row->status=='menunggu verifikasi sdm'){
                    $render = $level=='<a href="'.route('sppd.approval',['id'=>$row->id,'level'=>($level=="dosen" && $verifikasi? 'warek':$level??'-')]).'" class="btn btn-success"><i class="bi bi-check-lg"></i></a>';
                    $render .= '
                        <a href="#" class="ml-2 btn btn-danger btn-reject"><i class="bi bi-x-lg"></i></a>
                    ';
                }
