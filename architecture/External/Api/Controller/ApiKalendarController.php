...
class ApiKalendarController extends Controller //data cuti, izin, sppd, absen belum masuk
{
    ...
    public function index(Request $request, $tahun, $format = 'default')
    {
        ...
        try {
            ...
            $list_cuti = in_array($level, ["dosen", "pegawai"]) ? $this->queryBus->ask(new GetAllCutiQuery($nidn, $nip, 0, $tahun, TypeData::Default, false)) : collect([]);
            ...
            $list_sppd = in_array($level, ["dosen", "pegawai"]) ? $this->queryBus->ask(new GetAllSPPDQuery($nidn, $nip, 0, $tahun, TypeData::Default, false)) : collect([]);
