        ...
        <div class="col-lg-12">
            <div class="row">
                ...
                <div class="col-12">
                    <div class="card">
                        <div class="card-body">
                            <form action="{{ route('sppd.update') }}" method="post" enctype="multipart/form-data">
                                @csrf
                                <input type="hidden" name="id" class="@error('id') is-invalid @enderror" value="{{ old('id',$SPPD->GetId()) }}">
                                <div class="row">
                                    <div class="col-4">
                                        <x-input-text title="Tanggal Berangkat" name="tanggal_berangkat" class="tanggal_berangkat" default="{{ old('tanggal_berangkat',$SPPD->GetTanggalBerangkat()->toFormat(FormatDate::Default)) }}"/>
                                    </div>
                                    <div class="col-4">
                                        <x-input-text title="Tanggal Kembali" name="tanggal_kembali" class="tanggal_kembali" default="{{ old('tanggal_kembali',$SPPD->GetTanggalkembali()->toFormat(FormatDate::Default)) }}"/>
                                    </div>
                                    <div class="col-4">
                                        <x-input-select title="Jenis SPPD" name="jenis_sppd" class="jenis_sppd"></x-input-select>
                                    </div>
                                    <div class="col-12">
                                        <x-text title="Tujuan" name="tujuan" class="tujuan" default="{{ old('tujuan',$SPPD->GetTujuan()) }}"/>
                                    </div>
                                    <div class="col-12">
...
@push('scripts')
    ...
    <script>
        $(document).ready(function () {
            ...
            load_dropdown('.verifikasi', null, `{{ route('select2.PegawaiV2.List') }}?struktural=verifikator`, "{{ old('verifikasi',$SPPD->GetVerifikasi()?->GetNip()) }}", '-- Pilih Nama Atasan --');
            ...
            $('.tanggal_kembali').datepicker({
                ...
                datesDisabled:[],
                daysOfWeekDisabled:[],
                startDate:"{{old('tanggal_berangkat',$SPPD->GetTanggalBerangkat()?->toFormat(FormatDate::Default))}}",
                }).on('show', function(e) {
                // Mengatur posisi popover Datepicker ke center (middle).
                var $input = $(e.currentTarget);
                var $datepicker = $input.data('datepicker').picker;
